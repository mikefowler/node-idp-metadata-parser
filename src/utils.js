...
function getSingleSignonServiceBinding(doc, bindingPriority) {
  const nodes = doc.find('md:IDPSSODescriptor/md:SingleSignOnService/@Binding', NAMESPACE);
...
function getSingleLogoutServiceBinding(doc, bindingPriority) {
  const nodes = doc.find('md:IDPSSODescriptor/md:SingleLogoutService/@Binding', NAMESPACE);
...
export function getCertificateMetadata(certificates, fingerprintAlgorithm) {
  ...
  const {
    signing: signingCerts,
    encryption: encryptionCerts,
  } = certificates;
  ...
  if (certCount === 1 || hasTwoMatchingCerts) {
    ...
    const idpCertFingerprint = getCertificateFingerprint(idpCert, fingerprintAlgorithm);
